...
import static org.gradle.api.internal.tasks.execution.ResolveTaskOutputCachingStateExecuter.resolveCachingState
...
class ResolveTaskOutputCachingStateExecuterTest extends Specification {
    ...
    def "can turn caching off via cacheIf()"() {
        when:
        def state = resolve([spec({ false }, "Cacheable test")], [])
    ...
    def "can turn caching off via doNotCacheIf()"() {
        when:
        def state = resolve([spec({ true })], [spec({ true }, "Uncacheable test")])
    ...
    def "caching is disabled for non-cacheable file outputs is reported"() {
        ...
        then:
        !state.enabled
        state.disabledReason == "Output property 'prop' contains a file tree"
    ...
    def "caching is disabled when cache key is invalid because of invalid task implementation"() {
        ...
        when:
        def state = resolve([spec({ true })], [], [cacheableOutputProperty], invalidBuildCacheKey)
    ...
    def "caching is disabled when cache key is invalid because of overlapping outputs"() {
        ...
        when:
        def state = resolve([spec({ true })], [], [cacheableOutputProperty], cacheKey, overlappingOutputs)
    ...
    def resolve(
            List<SelfDescribingSpec<Spec<TaskInternal>>> cacheIf,
            List<SelfDescribingSpec<Spec<TaskInternal>>> doNotCacheIf,
            List<TaskOutputFilePropertySpec> outputProperties = [cacheableOutputProperty],
            TaskOutputCachingBuildCacheKey cacheKey = cacheKey,
            OverlappingOutputs overlappingOutputs = null
    ) {
        resolveCachingState(
                outputProperties,
                cacheKey,
    ...
    static def spec(Spec<TaskInternal> spec, String description = "test cacheIf()") {
        SelfDescribingSpec.of(spec, description)
    }
}
